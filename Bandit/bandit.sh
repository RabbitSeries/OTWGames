...
if [ $level = 13 ]; then
	...
	level=$((level+1))
	psw=$(ssh -i ./level13.sshkey "bandit$level@$BANDIT_DOMAIN" -p $BANDIT_PROT 'cat /etc/bandit_pass/bandit14' 2>>./exception.log)
	rm ./level13.sshkey
	print_psw_to_file $level "$psw"
fi
...
if [ $level = 24 ]; then
	psw=$(ssh_comand $level $psw 'rm -f /tmp/tmp_rabbit_bandit24 2>/dev/null; echo "$(for i in {0..9999}; do printf "'"${psw}"' %04d\n" $i; done)" | nc localhost 30002 | grep -oE "[[:alnum:]]{32}$" ' )
    level=$((level+1))
    print_psw_to_file $level "$psw"
fi

if [ $level = 25 ]; then
	ssh_comand $level $psw 'cat ./bandit26.sshkey' > bandit26.sshkey
	chmod 600 ./bandit26.sshkey
	echo "PLease resize your terminal's height below two visible lines for this level, press enter if you are all set"
	read
	# v
...
git_comand(){
	sshpass -p $2 git clone ssh://bandit$1-git@$BANDIT_DOMAIN:$BANDIT_PROT/home/bandit$1-git/repo 2>>exception.log
}

if [ $level = 27 ]; then
	git_comand $level $psw
	psw=$(extract_last_segment "$(cat ./repo/README)")
	rm -rf repo
    level=$((level+1))
    print_psw_to_file $level "$psw"
fi
